    ...
    while circumference > 100:
        ...
        def find_contour_line_intersections(cx, cy, angle_degs, contour_array, spacing):
            ...
            contour_array : np.ndarray
                Binary array where the contour is marked as 1
            spacing : tuple
                (x_spacing, y_spacing) in mm/pixel
            """
            ...
            # Get contour points
            y_indices, x_indices = np.where(contour_array > 0)
            contour_points = np.column_stack((x_indices, y_indices))  # Note: x, y order
            center = np.array([cx, cy])
            
            # Calculate angles from center to all contour points
        ...
        CVAI = ((longer_diag - smaller_diag)/longer_diag) * 100
        measurements = {
            'patient_id': patient_id,
            'iteration': k,
